...
const runBbsBenchmark = async (
  ...
  const messageSignRequest = await generateBbsSignRequest(bbsKeyPair, numberOfMessages, messageSizeInBytes);
...
const runBlsBenchmark = async (
  ...
  const blsKeyPair = await generateBls12381G2KeyPair();
  const MessageSignRequest = await generateBlsSignRequest(blsKeyPair, numberOfMessages, messageSizeInBytes);
